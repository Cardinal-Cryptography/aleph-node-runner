...
check_default_dir () {
    ...
    if [ -z "${LOCAL}" ] && [ ! -d "${HOST_BASE_PATH}/${DB_SNAPSHOT_PATH}" ] && [ -d "${DB_SNAPSHOT_PATH}/keystore" ]
    then
...
get_chainspec () {
    ...
    # For local testing we use what we have in this repo (which was generated by aleph-node bootstrap)
    if [ -z "${LOCAL}" ]
    then
        ...
        echo "Copying the local chainspec..."
        cp local_chainspec.json "${HOST_BASE_PATH}"
    fi
}
...
run_validator () {
    ...
    then
        source env/validator_local
        eval "echo \"$(cat env/validator_local)\"" > env/validator_local.env
        ENV_FILE="env/validator_local.env"
    else
...
run_archivist () {
    ...
    then
        source env/archivist_local 
        eval "echo \"$(cat env/archivist_local)\"" > env/archivist_local.env
        ENV_FILE="env/archivist_local.env"
    else
...
if [ -n "${LOCAL}" ] && [ -z "${ALEPH_IMAGE}" ]
then
    echo "When running a local node, you need to provide a docker image to run (run with --image option)"
    exit 1
fi
